...
<%
	...
	System.out.println("[debug] selectOrderList.jsp 로직 진입");	
    ...
    if(loginMember == null || loginMember.getMemberLevel() < 1) {
       ...
       System.out.println("[debug] selectOrderList.jsp => index.jsp로 강제 이동: 접근권한이 없는 이용자의 강제 접근을 막았습니다.");
       return;
    } 
    ...
    ArrayList<OrderEbookMember> oemList = orderDao.selectOrderList(beginRow, ROW_PER_PAGE, searchOrderNo);
			...
			<table class="table table-hover">
				<thead class="thead-light">	
					<tr class="align-content-center">
						...
						<th class="text-center" style="width: 20%" >주문 회원 아이디</th>
					</tr>
				</thead>
				<tbody>
					<% 
					...
					%>
							<tr>
								...
								<td class="text-center" style="width: 20%">
									<a href="<%=request.getContextPath() %>/admin/selectOrderOne.jsp?orderNo=<%=oem.getOrder().getOrderNo()%>"><%=oem.getEbook().getEbookTitle()%></a>
								</td>
								...
								<td class="text-center" style="width: 20%" ><%=oem.getMember().getMemberId()%></td>
							</tr>
					<% 
						}
					%>
							...
							<div class="text-center">
								...
								<ul class="pagination justify-content-center" style="margin:20px 0">
									<%		
									    ...
									    System.out.println("[debug] selectEbookList.jsp => 총 주문 수 : " + countOrderList);	
									...
									%>
											...
											<li class="page-item"><a class="page-link" href="<%=request.getContextPath() %>/admin/selectOrderList.jsp?currentPage=<%=1%>&searchOrderNo=<%=searchOrderNo%>">처음으로</a></li>
											<li class="page-item"><a class="page-link" href="<%=request.getContextPath() %>/admin/selectOrderList.jsp?currentPage=<%=rangeStartPage-9%>&searchOrderNo=<%=searchOrderNo%>">이전</a></li>
									<%
										}
									...
									%>	
											<li class="page-item"><a class="page-link" href="<%=request.getContextPath() %>/admin/selectOrderList.jsp?currentPage=<%=i%>&searchOrderNo=<%=searchOrderNo%>"><%=i%></a></li>
									<%		
										}
										...
										if((rangeEndPage) < lastPage) {
									%>
											<li class="page-item"><a class="page-link" href="<%=request.getContextPath() %>/admin/selectOrderList.jsp?currentPage=<%=rangeStartPage+11%>&searchOrderNo=<%=searchOrderNo%>">다음</a></li>
											<li class="page-item"><a class="page-link" href="<%=request.getContextPath() %>/admin/selectOrderList.jsp?currentPage=<%=lastPage%>&searchOrderNo=<%=searchOrderNo%>">끝으로</a></li>
									<%
										}
									%>
								</ul>
								<form class="form-group" method="get" action="<%=request.getContextPath() %>/admin/selectOrderList.jsp">
...
<%
	System.out.println("[debug] selectOrderList.jsp 로직 종료");	
%>